...
static float ndt_res = 1.5;      // Resolution
static double step_size = 0.05;   // Step size
static double trans_eps = 0.00001;  // Transformation epsilon
...
int main(int argc, char** argv)
{
  ...
  ros::Publisher scan_pub = nh.advertise<sensor_msgs::PointCloud2>("/current_scan", 1, true);
  ...
  correctLIDARscan(map, prev_vel, scan_interval);
  correctLIDARscan(scan, prev_vel, scan_interval);
  ...
  Eigen::Translation3f init_translation(-0.8, 0.1, 0);
